...
This script uses MarkItDown with Alibaba Cloud DashScope (via its OpenAI-compatible
endpoint) to generate detailed descriptions of images in documents (PowerPoint,
PDFs with images, etc.)
"""
...
def convert_with_ai(
    ...
    """
    try:
        # Initialize DashScope client (OpenAI-compatible endpoint)
        client = OpenAI(
            api_key=api_key,
...
Examples:
  ...
  python convert_with_ai.py slides.pptx slides.md --model qwen-vl-max --prompt-type presentation

  # Use custom prompt with fast vision model
  python convert_with_ai.py diagram.png diagram.md --model qwen-vl-plus --custom-prompt "Describe this technical diagram"
...
Environment Variables:
  DASHSCOPE_API_KEY    DashScope API key (required if not passed via --api-key)

Popular Models (use with --model):
  qwen-vl-max    - Recommended for vision tasks
  qwen-vl-plus   - Fast vision model
  qwen-max       - Text-only, high quality
        """
    )
    ...
    parser.add_argument(
        '--model', '-m',
        default='qwen-vl-max',
        help='Model to use via DashScope (default: qwen-vl-max)'
    )
    parser.add_argument(
        '--prompt-type', '-t',
    ...
    if not api_key:
        print("Error: DashScope API key required. Set DASHSCOPE_API_KEY environment variable or use --api-key")
