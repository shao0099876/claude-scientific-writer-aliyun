...
## Slide Generation with 通义万相

**This skill uses 通义万相 AI to generate stunning presentation slides automatically.**

There are two workflows depending on output format:

### Default Workflow: PDF Slides (Recommended)

Generate each slide as a complete image using 通义万相, then combine into a PDF. This produces the most visually stunning results.

**How it works:**
1. **Plan the deck**: Create a detailed plan for each slide (title, key points, visual elements)
2. **Generate slides**: Call 通义万相 for each slide to create complete slide images
...
2. **Always attach the previous slide** when generating subsequent slides using `--attach`:
   - This allows 通义万相 to see and match the existing style
...
5. **Attach existing figures/data for results slides** (CRITICAL for data-driven presentations):
   ...
   - Use `--attach` to include these figures so 通义万相 can incorporate them:
...
4. Describe how 通义万相 should incorporate the attached figures
...
When creating PowerPoint presentations, use 通义万相 to generate images and figures for each slide, then add text separately using the PPTX skill.

**How it works:**
1. **Plan the deck**: Create content plan for each slide
2. **Generate visuals**: Use 通义万相 with `--visual-only` flag to create images for slides
...
## 通义万相 Script Reference

### generate_slide_image.py

Generate presentation slides or visuals using 通义万相 AI.
...
# With reference images attached (通义万相 will see these)
...
Use `--attach` when you want 通义万相 to see existing images as context:
...
Use 通义万相 with `--visual-only` to generate images, then build PPTX with text.
...
2. **Generate Slides with 通义万相** (1-2 hours):
